...
type (
	...
	Provider struct {
		...
		evaluationFunc     evaluationFn[of.FlagTypes]
		logger             *slog.Logger
	...
	configuration struct {
		...
		runMode          runModeFn[of.FlagTypes]
	}
...
func WithRunModeParallel() Option {
	return func(conf *configuration) {
		conf.runMode = runModeParallel[of.FlagTypes]
	}
}
...
func NewProvider(evaluationStrategy EvaluationStrategy, options ...Option) (*Provider, error) {
	config := &configuration{
		...
		runMode:   runModeSequential[of.FlagTypes],
	}
	...
	var strategy StrategyFn[of.FlagTypes]
...
func newEvaluationFunc[T of.FlagTypes](providers []namedProvider, runMode runModeFn[T], strategy StrategyFn[T]) evaluationFn[T] {
...
// ObjectEvaluation evaluates the flag and returns an [of.ObjectResolutionDetail]. For the purposes of evaluation
...
func (p *Provider) Init(ctx context.Context, evalCtx of.EvaluationContext) error {
	...
	for _, provider := range p.providers {
		...
		eg.Go(func() error {
			...
			if stateHandle, ok := tryAs[of.StateHandler](prov); ok {
				err := stateHandle.Init(ctx, evalCtx)
				if err != nil {
...
func (p *Provider) Shutdown(ctx context.Context) error {
	...
	for _, provider := range p.providers {
		...
		if stateHandle, ok := tryAs[of.StateHandler](provider); ok {
			meg.Go(func() error {
				if err := stateHandle.Shutdown(ctx); err != nil {
